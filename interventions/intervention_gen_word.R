...
for (i in 1:nrow(dat)) {
  ...
  # dat$Measure[i] = with(dat[i, ], 
  #      exam(outcome_test, outcome_report, outcome_observation,  type = "test"))
}
...
Implent_setting_text = with(dat, paste0(
  ...
  ifelse(is.na(dat$INTER_dose), 
         "No specific information on dosage was available. ", paste0(
  "Regarding dosage/intensity, we found that ", Interventions, 
