...
$(document).ready(function () {
  ...
  $("#data-table").load(
    ...
    function () {
      ...
      //   if ($.inArray(type, typeOptions) == -1) typeOptions.push(type);
      // });
      // typeOptions.sort();
      ...
      //   );
      // });
      var typeOptions = ["Psychosocial", "Complementary"];
      $.each(typeOptions, function (i, value) {
        $("#filter1").append(
          '<option value="' + value + '">' + value + "</option>"
        );
      ...
      // Populate the second filter with the predefined age options
      var ageOptions = [
        "Very young children",
        "School-age children",
        "Adolescents",
      ...
      var outcomeOptions = [
        ...
        // Define more groups and options as needed
      ];
      ...
      $.each(outcomeOptions, function (i, group) {
        ...
        // Loop through options within the group
        $.each(group.options, function (j, option) {
          // Append the option within the group
          $("#filter3").append(
            '<option value="' + option + '">' + option + "</option>"
          );
        });

        // Close the optgroup tag after all options in the group
        $("#filter3").append("</optgroup>");
      }); // var outcomeOptions = [
      // ];
      // $.each(outcomeOptions, function (i, value) {
      //   $("#filter3").append(
      //     '<option value="' + value + '">' + value + "</option>"
      //   );
      // });
